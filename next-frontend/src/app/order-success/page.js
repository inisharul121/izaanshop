        ...
        <div className="space-y-6">
          <div className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="text-sm text-gray-600 space-y-2">
              <div className="font-extrabold text-dark text-lg mb-1">{order.user?.name || order.guestName}</div>
              <div className="text-gray-500 leading-relaxed font-medium">{order.street}</div>
              {order.city && order.city !== 'N/A' && <div className="text-gray-500 font-medium">{order.city}{order.zipCode ? `, ${order.zipCode}` : ''}</div>}
